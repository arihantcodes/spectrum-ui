...
import { ChartEyebrow, ChartSurface, useChartMotion } from './chart-kit';
...
export function CohortRetentionChart({ cohorts = SAMPLE, className }: CohortRetentionChartProps) {
  ...
  return (
    ...
    >
      <div className="relative z-10 flex items-end justify-between gap-3">
        <div>
          <ChartEyebrow>Retention</ChartEyebrow>
          <p className="mt-2 text-[1.85rem] leading-none font-semibold tabular-nums tracking-tighter text-neutral-900 dark:text-neutral-50">
            {readout.value ?? '—'}%
          </p>
          <p className="mt-1 text-[11px] text-neutral-400 dark:text-neutral-500">
            {readout.cohort} · week {readout.week}
          </p>
        </div>
      </div>
      ...
      <div className="relative z-10 mt-5 overflow-x-auto">
        <div
          ...
          style={{ gridTemplateColumns: `3.5rem repeat(${weeks}, minmax(2.2rem, 1fr))` }}
        >
          <span />
          {Array.from({ length: weeks }, (_, col) => (
            <span
              key={`h-${col}`}
              className={cn(
                'text-center text-[10px] tracking-wide text-neutral-400 dark:text-neutral-500',
                hover?.col === col && 'font-semibold text-neutral-700 dark:text-neutral-200',
              )}
            >
              W{col}
            </span>
          ...
          {cohorts.map((row, rowIndex) => (
            <React.Fragment key={row.label}>
              <span
                className={cn(
                  'pr-2 text-right text-[11px] leading-8 text-neutral-500 dark:text-neutral-400',
                  hover?.row === rowIndex && 'font-semibold text-neutral-800 dark:text-neutral-200',
                )}
              >
                {row.label}
              </span>
              {Array.from({ length: weeks }, (_, col) => {
                ...
                const highlight = hover != null && (hover.row === rowIndex || hover.col === col);
                const focus = hover?.row === rowIndex && hover?.col === col;
                const mix = col === 0 ? 18 : 18 + (value / 100) * 82;
                return (
                  <motion.button
                    key={`${row.label}-${col}`}
                    type="button"
                    ...
                    animate={{
                      opacity: empty ? 0 : highlight || hover == null ? 1 : 0.28,
                      scale: focus ? 1.07 : 1,
                    }}
                    transition={{
                    ...
                    className={cn(
                      'h-8 rounded-lg text-[10px] font-semibold tabular-nums tracking-tight',
                      'focus-visible:ring-1 focus-visible:ring-neutral-950 focus-visible:outline-hidden dark:focus-visible:ring-neutral-300',
                      col === 0 && 'text-neutral-500 dark:text-neutral-400',
                      col !== 0 && 'text-neutral-950 dark:text-neutral-50',
                    )}
                    style={
                      empty
                        ? undefined
                        : {
                            backgroundColor: `color-mix(in oklab, var(--chart-solid) ${Math.round(mix)}%, transparent)`,
                            boxShadow: focus ? '0 0 18px var(--chart-glow)' : 'none',
                          }
                    }
                  >
                    {empty ? '' : value}
