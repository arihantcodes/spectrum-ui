...
const TILES = [
  ...
  { href: '/docs/charts/infra-trio', name: 'Infra Trio', node: <InfraTrioChart /> },
  {
    ...
    name: 'Activity Lattice',
    node: <ActivityLatticeChart />,
  },
  {
    href: '/docs/charts/conversion-cascade',
    name: 'Conversion Cascade',
    node: <ConversionCascadeChart />,
  },
  {
    href: '/docs/charts/cohort-retention',
    name: 'Cohort Retention',
    node: <CohortRetentionChart />,
  },
] as const;
  ...
  return (
    <div className="mt-10 grid gap-8 lg:grid-cols-2">
      {TILES.map((tile, index) => (
        <article key={tile.href} className={index === 0 ? 'min-w-0 lg:col-span-2' : 'min-w-0'}>
          <Link
            href={tile.href}
