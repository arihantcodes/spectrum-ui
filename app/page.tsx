...
const HomeCardCollection = dynamic(() => import('@/components/homecard'), {
  ssr: true,
  loading: () => (
    <div className="animate-pulse h-[400px] w-full bg-neutral-100 dark:bg-neutral-900 rounded-lg"></div>
  ),
});
...
const Homepage = () => {
  return (
    // container-frame = the site frame (shared max-width, no rules drawn). `@container` lets the
    // hero stage measure the page width (100cqw) and bleed to the viewport edge.
    <div className="@container">
      <div className="container-frame">
        <HeroSection />
      </div>

      {/* The tint bleeds to the viewport edges: a tinted box with hard side
          edges would read as another frame on a borderless page. */}
